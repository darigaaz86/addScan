...
func (qc *QueryCache) ExecuteMultiAddressQuery(ctx context.Context, addresses []string, params *QueryParams) (*QueryResult, error) {
	...
	// Query each address and combine results
	var transactions []*models.Transaction
	for _, addr := range addresses {
		txs, err := qc.repository.GetByAddress(ctx, addr, filters)
		if err != nil {
			return nil, fmt.Errorf("failed to query transactions for %s: %w", addr, err)
		}
...
func (qc *QueryCache) GetTransactionByHash(ctx context.Context, hash string) ([]*models.Transaction, error) {
	...
	found, err := qc.cache.Get(ctx, cacheKey, &txs)
	if err != nil {
		return nil, fmt.Errorf("cache get error: %w", err)
	}
	...
	// Cache miss - query ClickHouse
	transactions, err := qc.repository.GetByHash(ctx, hash)
	if err != nil {
	...
	if err := qc.cache.SetWithTTL(ctx, cacheKey, transactions, 24*time.Hour); err != nil {
